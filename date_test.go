...
func TestDate(t *testing.T) {
	...
	user := UserWithDate{Name: "jinzhu", Date: datatypes.Date{Time: time.Now()}}
	DB.Create(&user)

	result := UserWithDate{}
	if err := DB.First(&result, "name = ? AND date = ?", "jinzhu", datatypes.Date{Time: time.Now()}).Error; err != nil {
...
func TestGobEncoding(t *testing.T) {
	date := datatypes.Date{Time: time.Now()}
